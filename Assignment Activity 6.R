...
# Loading Data
sales_data <- read.csv("/Users/orkestro/Downloads/LSE_DA301_assignment_files/turtle_sales.csv")
...
model_simple <- lm(Global_Sales ~ NA_Sales, data = sales_data)

# Viewing the Output
summary(model_simple)

# Correlation Matrix
sales_correlation <- cor(sales_data[, c("NA_Sales", "EU_Sales", "Global_Sales")], use = "complete.obs")

# Visualizing the Correlation
corrplot(sales_correlation, method="circle")

# Scatterplot with Regression Line
ggplot(sales_data, aes(x=NA_Sales, y=Global_Sales)) +
  geom_point() +
  geom_smooth(method="lm", col="blue") +
  labs(title="Regression of Global Sales on North American Sales",
       x="North American Sales (in millions)",
...
# Step 1: Create a Multiple Linear Regression Model

model_mult <- lm(Global_Sales ~ NA_Sales + EU_Sales, data = data)

# Step 2: Select Only the Numeric Columns

numeric_data <- select_if(data, is.numeric)

# Step 3: Determine the Correlation Between Sales Columns

sales_correlation <- cor(numeric_data[,c("NA_Sales", "EU_Sales", "Global_Sales")])

# Viewing the correlation matrix
print(sales_correlation)

# Fitting the model using 'sales_data'
model_mult <- lm(Global_Sales ~ NA_Sales + EU_Sales, data = sales_data)
